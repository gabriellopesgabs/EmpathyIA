...
impl ParakeetEngine {
    ...
    pub async fn download_model_detailed(
        ...
        let expected_hashes: HashMap<&str, &str> = if model_name.contains("-v2-") {
            [
                (
                    "encoder-model.int8.onnx",
        ...
        for (index, filename) in files_to_download.iter().enumerate() {
            ...
            if existing_size >= size_tolerance
                && expected_size > 0
                && verify_sha256(&file_path, expected_hashes.get(*filename).copied()).await?
            {
                log::info!(
            ...
            if !verify_sha256(&file_path, expected_hashes.get(*filename).copied()).await? {
                let _ = fs::remove_file(&file_path).await;
                let mut active = self.active_downloads.write().await;
                active.remove(model_name);
                return Err(anyhow!(
                    "Integrity verification failed for {}. The downloaded file was removed.",
                    filename
                ));
            }
...
async fn verify_sha256(path: &PathBuf, expected: Option<&str>) -> Result<bool> {
    let Some(expected) = expected else {
        return Err(anyhow!(
            "No trusted checksum configured for {}",
            path.display()
        ));
    };
