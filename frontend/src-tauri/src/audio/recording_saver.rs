...
impl RecordingSaver {
    ...
    fn write_transcripts_json(&self, folder: &PathBuf) -> Result<()> {
        ...
        let markdown_segments = segments_clone
            .iter()
        ...
        crate::meeting_files::write_transcript(
            ...
            self.metadata
                .as_ref()
                .and_then(|metadata| metadata.meeting_id.as_deref()),
            self.meeting_name.as_deref().unwrap_or("Reunião"),
            &chrono::Utc::now().to_rfc3339(),
            &markdown_segments,
        )?;
        info!(
            "✅ Successfully wrote {} with {} segments",
            crate::meeting_files::TRANSCRIPT_FILE,
            markdown_segments.len()
        );
        Ok(())
    }
