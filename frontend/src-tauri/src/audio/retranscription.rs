...
async fn run_retranscription<R: Runtime>(
    ...
    .await
    .map_err(|error| anyhow!("Failed to load meeting metadata: {}", error))?
    .ok_or_else(|| anyhow!("Meeting not found after retranscription: {}", meeting_id))?;
    let now = chrono::Utc::now().to_rfc3339();
    let markdown_segments = segments
        .iter()
    ...
    .map_err(|error| anyhow!("Failed to save canonical Markdown: {}", error))?;
