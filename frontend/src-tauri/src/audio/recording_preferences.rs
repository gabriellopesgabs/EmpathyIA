...
pub fn get_default_recordings_folder() -> PathBuf {
    // Use ~/EmpathyAI on all platforms (e.g. /Users/gabriel/EmpathyAI)
    dirs::home_dir()
...
pub async fn get_available_audio_backends() -> Result<Vec<String>, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        Ok(backends.iter().map(AudioCaptureBackend::id).collect())
    }
...
pub async fn get_audio_backend_info() -> Result<Vec<BackendInfo>, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        let backends = vec![
            BackendInfo {
                id: AudioCaptureBackend::ScreenCaptureKit.id(),
            ...
            BackendInfo {
                id: AudioCaptureBackend::CoreAudio.id(),
