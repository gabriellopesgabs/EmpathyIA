...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    let (auto_save, preferred_mic_name, preferred_system_name, obsidian_vault_path) =
        match super::recording_preferences::load_recording_preferences(&app).await {
            Ok(prefs) => {
                info!("📋 Loaded recording preferences: auto_save={}, preferred_mic={:?}, preferred_system={:?}",
                      prefs.auto_save, prefs.preferred_mic_device, prefs.preferred_system_device);
                (prefs.auto_save, prefs.preferred_mic_device, prefs.preferred_system_device, prefs.obsidian_vault_path.flatten())
            }
            Err(e) => {
    ...
    let meeting_start_time = chrono::Local::now();
    // Always ensure a meeting name is set so incremental saver initializes
    let effective_meeting_name = meeting_name.clone().unwrap_or_else(|| {
        // Example: Meeting 2025-10-03_08-25-23
        format!(
            "Meeting {}",
            meeting_start_time.format("%Y-%m-%d_%H-%M-%S")
        )
    });
    ...
    crate::audio::diarization::reset_diarizer(); // Reset speaker clusters for new session
    ...
    let obsidian_vault_path_clone = obsidian_vault_path.clone();
    let meeting_name_clone = effective_meeting_name.clone();
    let meeting_date_clone = meeting_start_time.to_rfc3339();
...
pub async fn start_recording_with_devices_and_meeting<R: Runtime>(
    ...
    let (auto_save, obsidian_vault_path) = match super::recording_preferences::load_recording_preferences(&app).await {
        Ok(prefs) => {
            info!("📋 Loaded recording preferences: auto_save={}", prefs.auto_save);
            (prefs.auto_save, prefs.obsidian_vault_path.flatten())
        }
        Err(e) => {
    ...
    let obsidian_vault_path_clone = obsidian_vault_path.clone();
    let meeting_name_clone = effective_meeting_name.clone();
    let meeting_date_clone = chrono::Local::now().to_rfc3339();
