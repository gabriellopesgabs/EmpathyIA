                    ...
                    if let Ok(prefs) = crate::audio::recording_preferences::load_recording_preferences(&app).await {
                        if let Some(Some(ref vault_path)) = prefs.obsidian_vault_path {
                            ...
                            if let Ok(Some(meeting)) = MeetingsRepository::get_meeting(&pool, &meeting_id).await {
                                if let Ok((transcripts, _)) = MeetingsRepository::get_meeting_transcripts_paginated(&pool, &meeting_id, 1000000, 0).await {
                                    ...
                                    if let Err(e) = crate::audio::obsidian::save_to_obsidian_vault(vault_path, &meeting.title, &md_content) {
                                        warn!("Failed to update Obsidian vault note with summary: {}", e);
                                    } else {
