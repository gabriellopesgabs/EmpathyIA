    ...
    pub async fn process_transcript_background<R: tauri::Runtime>(
        ...
        match result {
            Ok((final_markdown, english_markdown, num_chunks)) => {
                ...
                let canonical_summary = result_json.clone();
                if let Ok(Some(meeting)) =
                    MeetingsRepository::get_meeting_metadata(&pool, &meeting_id).await
                {
                    ...
                    {
                        if let Err(error) = crate::meeting_files::write_summary(
                            ...
                            let message =
                                format!("Canonical summary Markdown could not be saved: {}", error);
                ...
                .await
                {
                    ...
                    {
                        if let Some(Some(ref vault_path)) = prefs.obsidian_vault_path {
                            // Fetch meeting details and transcripts
                            if let Ok(Some(meeting)) =
                                MeetingsRepository::get_meeting(&pool, &meeting_id).await
                            {
                                if let Ok((transcripts, _)) =
                                    MeetingsRepository::get_meeting_transcripts_paginated(
                                        &pool,
                                        &meeting_id,
                                        1000000,
                                        0,
                                    ...
                                    .await
                                {
                                    let export_transcripts = transcripts
                                        .into_iter()
                                        .map(|t| crate::audio::obsidian::ExportTranscriptSegment {
                                            speaker: t.speaker,
                                            text: t.transcript,
                                    ...
                                    let export_data = crate::audio::obsidian::ObsidianExportData {
                                        ...
                                        speakers,
                                        summary_markdown: Some(final_markdown.clone()),
                                        transcripts: export_transcripts,
                                    };

                                    let md_content =
                                        crate::audio::obsidian::generate_obsidian_markdown(
                                            &export_data,
                                        );
