  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm relative">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-lg font-semibold text-gray-900">Idioma dos resumos</h3>
      </div>
      <p className="text-sm text-gray-600 mb-4">
        Escolha um idioma padrão ou deixe o Empathy acompanhar o idioma predominante da transcrição.
      </p>
        ...
        {recents.map((code) => {
          ...
          return (
            ...
            >
              <button
                type="button"
                aria-label={isPinned ? `Remover ${labelForCode(code)} como padrão` : `Usar ${labelForCode(code)} como padrão`}
                aria-pressed={isPinned}
                title={isPinned ? 'Remover como padrão' : 'Definir como padrão'}
      ...
      <p className="text-xs text-gray-400 mt-3">
        {pinned
          ? `Padrão: ${labelForCode(pinned)}. Selecione novamente para remover.`
          : 'Selecione um idioma para torná-lo padrão. Até cinco opções rápidas.'}
      </p>
    </div>
  );
}
