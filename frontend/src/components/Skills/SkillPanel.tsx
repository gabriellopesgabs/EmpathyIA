...
import type { RelatedSkillDocument, SkillDefinition, SkillInfo, SkillLayer, SkillRunResult } from '@/types/skills';
...
export function SkillPanel({ open, note, content, selection, transcript, onClose, onAccept }: {
  open: boolean; note: NoteDocument; content: string; selection?: TextSelection | null; transcript?: string;
  onClose: () => void; onAccept: (result: SkillRunResult, title: string, markdown: string) => void;
}) {
  ...
  const filtered = useMemo(() => skills.filter(skill => layer === 'all' || skill.layer === layer), [layer, skills]);
  ...
  useEffect(() => { setPrivacyAccepted(false); }, [selectedId, useTranscript, relatedIds]);
  if (!open) return null;
  ...
  const run = async () => {
    ...
    try {
      ...
      const generated = await invoke<SkillRunResult>('api_run_skill', { skillId: selected.id, context: {
        note_id: note.id, note_title: note.title, note: content, selection: selection?.text || null,
        transcript: useTranscript ? transcript || null : null, related_notes: relatedDocuments,
      } });
  ...
  return (
    <aside className="skill-panel" aria-label="Painel de Skills">
      ...
      {!result && !editor && <>
        ...
        <div className="skill-list">{filtered.map(skill => <button key={skill.id} data-active={selectedId === skill.id} onClick={() => setSelectedId(skill.id)}><span data-layer={skill.layer} /><strong>{skill.name}</strong><small>{skill.description}</small></button>)}</div>
        {selected && <section className="skill-context">
          <div><strong>Contexto desta execução</strong><small>{selection?.text ? `Seleção: “${selection.text.slice(0, 100)}${selection.text.length > 100 ? '…' : ''}”` : 'Nota completa'}</small></div>
          {selected.context.transcript && transcript && <label><input type="checkbox" checked={useTranscript} onChange={event => setUseTranscript(event.target.checked)} /> Incluir transcrição</label>}
        ...
        <div className="skill-privacy" data-external={isExternal}><strong>{isExternal ? 'Processamento externo' : 'Processamento local'}</strong><span>{modelConfig.provider} · {modelConfig.model}</span><p>{isExternal ? `Será enviado: ${selection?.text ? 'seleção' : 'nota completa'}${useTranscript ? ', transcrição' : ''}${relatedIds.length ? ` e ${relatedIds.length} nota(s) relacionada(s)` : ''}.` : 'O conteúdo permanece neste dispositivo.'}</p>{isExternal && <label><input type="checkbox" checked={privacyAccepted} onChange={event => setPrivacyAccepted(event.target.checked)} /> Confirmo o envio deste contexto</label>}</div>
