...
export function PreferenceSettings() {
  ...
  const [recordingPrefs, setRecordingPrefs] = useState<any>(null);

  useEffect(() => {
    const fetchRecordingPrefs = async () => {
      try {
        const prefs = await invoke<any>('get_recording_preferences');
        setRecordingPrefs(prefs);
        if (prefs && prefs.obsidian_vault_path) {
  ...
  const handleSelectVaultFolder = async () => {
    try {
      const selected = await invoke<string | null>('api_select_folder');
      if (selected) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">Avisar quando uma gravação começar ou terminar.</p>
          </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Arquivos e armazenamento</h3>
        <p className="text-sm text-gray-600 mb-6">
          Seus dados permanecem em arquivos Markdown que você pode abrir e copiar.
        </p>
        ...
        <div className="space-y-4">
          ...
          {/* Recordings Location */}
          <div className="p-4 border rounded-lg bg-gray-50">
            <div className="font-medium mb-2">Pasta do Empathy</div>
            <div className="text-sm text-gray-600 mb-3 break-all font-mono text-xs">
          ...
          <div className="p-4 border rounded-lg bg-gray-50">
            <div className="font-medium mb-2">Pasta Markdown externa (opcional)</div>
            <div className="text-sm text-gray-600 mb-3 break-all font-mono text-xs">
              {obsidianVaultPath || 'Nenhuma pasta externa configurada'}
            </div>
            <button
            ...
            >
              <FolderOpen className="w-4 h-4" />
              Escolher pasta Markdown
            </button>
            <p className="mt-2 text-xs text-gray-500">
              Cria notas portáteis compatíveis com Obsidian e outros editores Markdown.
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 p-3 bg-blue-50 rounded-md">
          <p className="text-xs text-blue-800">
            <strong>Privacidade:</strong> o conteúdo das notas fica nos arquivos Markdown escolhidos por você. O índice local pode ser reconstruído.
          </p>
        </div>
      </div>
    </div>
