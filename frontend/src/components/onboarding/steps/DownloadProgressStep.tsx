...
export function DownloadProgressStep() {
  ...
  const handleRetryDownload = async () => {
    ...
    try {
      ...
      toast.error('Não foi possível retomar o download', {
  ...
  const handleRetrySummaryDownload = async () => {
    ...
    try {
      ...
      if (!modelName) {
        throw new Error('A recomendação do modelo de resumo ainda não está pronta');
      }
      ...
      toast.error('Não foi possível retomar o download do modelo', {
        description: 'Verifique sua conexão e tente novamente.',
      });
    } finally {
      // Allow retry again after 2 seconds
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (actuallyAvailable && !parakeetDownloaded) {
        ...
        toast.error('O reconhecimento de fala é necessário', {
          description: 'Tente o download novamente antes de continuar.',
        });
        return;
      }
    } catch (error) {
    ...
    if (!downloadsComplete) {
      toast.info('Os downloads continuarão em segundo plano', {
        description: 'Você já pode usar o app. A gravação será liberada quando o reconhecimento de fala estiver pronto.',
        duration: 5000,
      });
    }
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      ...
      {state.status === 'error' && state.error && (
        <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-md">
          <p className="text-sm text-red-600 font-medium">Erro no download</p>
          <p className="text-xs text-red-500 mt-1">{state.error}</p>
          {(title === 'Reconhecimento de fala' || title === 'Modelo de resumo') && (
            <button
              onClick={title === 'Reconhecimento de fala' ? handleRetryDownload : handleRetrySummaryDownload}
  ...
  return (
    <OnboardingContainer
      ...
      description="You can start using Empathy.AI after downloading the Transcription Engine."
      step={3}
          ...
          {parakeetDownloaded && !summaryModelDownloaded && (
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium">Você pode continuar enquanto isso termina</p>
