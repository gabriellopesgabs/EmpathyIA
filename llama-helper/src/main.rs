...
impl ModelState {
    ...
    fn generate(
        ...
        if let Some(ref g_str) = grammar {
            let root = grammar_root.as_deref().unwrap_or("root");
            match LlamaSampler::grammar(model, g_str, root) {
                Ok(g_sampler) => {
                    samplers.push(g_sampler);
                    eprintln!("✅ Grammar sampler initialized successfully");
                }
                Err(e) => {
                    eprintln!("⚠️ Failed to initialize grammar sampler: {:?}", e);
                }
            }
        }
